...
function bomberMan(n, grid) {
    // caso base: segundo 1 -> sin cambios
  if (n === 1) return grid;

  // caso par: el campo esta completamente lleno de bombas
  if (n % 2 === 0) return grid.map(row => 'O'.repeat(row.length));

  // funcion que simula una explosion
  const explode = (matriz) => {
    const rows = matriz.length;
    const cols = matriz[0].length;
    const result = Array.from({ length: rows }, () => Array(cols).fill('O'));
  ...
  // patron ciclico: repite cada 4 segundos despues del 3
  const matriz = grid.map(row => row.split(''));
  if ((n - 3) % 4 === 0) return explode(matriz);
  else return explode(explode(matriz));
}
// Example usage:
const initialGrid = [
  '.......',
...
const seconds = 3;
const resultGrid = bomberMan(seconds, initialGrid);